...
for F in ${RAW_READS}/*_R1_001.fastq.gz; do 
	...
	if [[ "$SAMPLE" =~ "_HC_" ]];then
		echo "##################################"
		echo "##################################"
		echo "Filter Unpaired reads in HiC"
		echo $SAMPLE
		echo "##################################"
		echo "##################################"
		java -jar ${TRIMM} PE -threads 16 -phred33 ${prc_data}/${SAMPLE}_1.fastq.gz ${prc_data}/${SAMPLE}_2.fastq.gz ${prc_data}/${SAMPLE}_1_paied.fastq.gz ${prc_data}/${SAMPLE}_1_single.fastq.gz ${prc_data}/${SAMPLE}_2_paired.fastq.gz ${prc_data}/${SAMPLE}_2_single.fastq.gz MINLEN:20
		rm ${prc_data}/${SAMPLE}_1_single.fastq.gz ${prc_data}/${SAMPLE}_2_single.fastq.gz
	else if [[ "$SAMPLE" =~ "_SG_" ]];then
		...
		echo $SAMPLE
		echo "##################################"
		idba_ud -r ${prc_data}/${SAMPLE}.fa --num_threads 12 --pre_correction -o ${ASSM}/IDBAUD/${SAMPLE}
