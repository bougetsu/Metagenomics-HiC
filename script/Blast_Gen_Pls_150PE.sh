...
for file in ${ASM}/*583*contigs.fa; do
	SAMPLE=${file##*/}
	BASE=${SAMPLE%.contigs.fa*}
	echo $BASE
	echo ${file}
	blastn -query ${file} -db ${V583}/V583 -perc_identity 90 -num_threads 8  -outfmt '6 qseqid sseqid pident mismatch gapopen qlen slen length qstart qend sstart send evalue' -out ${ASM}/blast_result/${BASE}_genome.tsv
	blastn -query ${file} -db ${PLASMID_FINDER}/plasmidfinder_db/AllRep -perc_identity 90 -num_threads 8 -outfmt '6 qseqid sseqid pident mismatch gapopen qlen slen length qstart qend sstart send evalue' -out ${ASM}/blast_result/${BASE}_rep.tsv
	blastn -query ${file} -db ${PLS} -perc_identity 90 -num_threads 8 -outfmt '6 qseqid sseqid pident mismatch gapopen qlen slen length qstart qend sstart send evalue' -out ${ASM}/blast_result/${BASE}_plsdb.tsv
done	

for file in ${ASM}/*HIP*contigs.fa; do
	SAMPLE=${file##*/}
	BASE=${SAMPLE%.contigs.fa*}
	echo $BASE
	echo ${file}
	blastn -query ${file} -db ${HIP}/HIP11704 -perc_identity 90 -num_threads 8 -outfmt '6 qseqid sseqid pident mismatch gapopen qlen slen length qstart qend sstart send evalue' -out ${ASM}/blast_result/${BASE}_genome.tsv
	blastn -query ${file} -db ${PLASMID_FINDER}/plasmidfinder_db/AllRep -perc_identity 90 -num_threads 8 -outfmt '6 qseqid sseqid pident mismatch gapopen qlen slen length qstart qend sstart send evalue' -out ${ASM}/blast_result/${BASE}_rep.tsv
	blastn -query ${file} -db ${PLS} -perc_identity 90 -num_threads 8 -outfmt '6 qseqid sseqid pident mismatch gapopen qlen slen length qstart qend sstart send evalue' -out ${ASM}/blast_result/${BASE}_plsdb.tsv
done
...
for file in ${ASM}/*VE*contigs.fa; do
	SAMPLE=${file##*/}
	BASE=${SAMPLE%.contigs.fa*}
	echo $BASE
	echo ${file}
	blastn -query ${file} -db ${HIP}/HIP11704 -perc_identity 90 -num_threads 8 -outfmt '6 qseqid sseqid pident mismatch gapopen qlen slen length qstart qend sstart send evalue' -out ${ASM}/blast_result/${BASE}_HIP.tsv
	blastn -query ${file} -db ${V583}/V583 -perc_identity 90 -num_threads 8  -outfmt '6 qseqid sseqid pident mismatch gapopen qlen slen length qstart qend sstart send evalue' -out ${ASM}/blast_result/${BASE}_V583.tsv
	blastn -query ${file} -db ${PLASMID_FINDER}/plasmidfinder_db/AllRep -perc_identity 90 -num_threads 8 -outfmt '6 qseqid sseqid pident mismatch gapopen qlen slen length qstart qend sstart send evalue' -out ${ASM}/blast_result/${BASE}_rep.tsv
	blastn -query ${file} -db ${PLS} -perc_identity 90 -num_threads 8 -outfmt '6 qseqid sseqid pident mismatch gapopen qlen slen length qstart qend sstart send evalue' -out ${ASM}/blast_result/${BASE}_plsdb.tsv
done
...
for file in ${ASM}/blast_result/*.tsv;do
	SAMPLE=${file##*/}
	BASE=${SAMPLE%.tsv}
